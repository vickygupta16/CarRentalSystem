<html>
<head>
<title>User Password Change Status</title>
...
<?php
    ...
    $uid=$_POST['uid'];
    $pw=$_POST['pw'];
    $temp="select * from userprofile where uid='$uid'";
    ...
    if(mysqli_num_rows($retval)>0)
    {
        while($row=mysqli_fetch_assoc($retval)>0)
        {
            if($uid!=$row['uid'])
            {
                $sql="update userprofile set password='$pw' where uid='$uid'";
    ...
    {
        echo "<h4 align=center>User ID $uid does not exist.</h4>";
    }
    mysqli_close($conn);
	?>
</body>
